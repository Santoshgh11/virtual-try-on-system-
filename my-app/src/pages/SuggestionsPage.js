...
import { getOutfitSuggestions } from '../services/api'; // ✅ Import outfit suggestions API
...
const SuggestionsPage = () => {
  ...
  // ✅ Extract user preferences & scan data from state
  const preferences = location.state?.preferences || ['casual', 'formal'];
  const scanResponse = location.state?.scanResponse;
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      try {
        ...
        if (scanResponse) {
          // ✅ Fetch AI outfit recommendations based on scanned data
          response = await getOutfitSuggestions(scanResponse);
        } else {
          // ✅ Fallback scan data (if none provided)
          const defaultScanData = {
            height: 170,
            weight: 65,
            dimensions: { chest: 90, waist: 70, hips: 95 },
        ...
        // ✅ Store AI outfit recommendations
        setSuggestions(response.recommendations?.outfits || []);
        setLoading(false);
      } catch (error) {
        console.error('Error fetching outfit suggestions:', error);
        setErrorMessage('Failed to load outfit suggestions. Please try again.');
  ...
  return (
    <div className="min-h-screen bg-gray-200 flex flex-col items-center p-8">
      <h1 className="text-4xl font-bold mb-8 text-gray-800">AI-Generated Outfit Suggestions</h1>

      {loading ? (
        <p className="text-lg text-gray-600">Fetching AI recommendations...</p>
      ) : errorMessage ? (
        <p className="text-red-600 font-semibold">{errorMessage}</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {suggestions.map((item, index) => (
            <div key={index} className="bg-white shadow-lg rounded-lg overflow-hidden max-w-sm">
              <img src={item.image} alt={item.name} className="w-full h-64 object-cover" />
              <div className="p-4">
                <h3 className="text-lg font-semibold text-gray-700">{item.name}</h3>
